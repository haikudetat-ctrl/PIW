...
| `PAID_PROVIDERS_ENABLED` | `false` until a provider is wired up | `false` (enforced — preview cannot enable paid providers) |
...
both `process-diagnostic-event` and `publish-outbox` registered. If it does
