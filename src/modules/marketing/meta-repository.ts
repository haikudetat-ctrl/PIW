...
export class SupabaseMetaRepository {
  ...
  async complete(
    ...
    const canTerminallyExhaust = parsedResult.outcome === "retryable_failed" && row?.status === "permanent_failed";
    if (error || !row || row.id !== deliveryId || (row.status !== parsedResult.outcome && !canTerminallyExhaust)) {
