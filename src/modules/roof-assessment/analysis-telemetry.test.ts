...
describe("assessment analysis telemetry", () => {
  test("derives a stable opaque correlation from the exact consumed attempt submission", async () => {
    ...
    expect(scope).toEqual({
      correlation: createAssessmentJourneyCorrelation(submissionId, signingSecret),
    });
  ...
  test("a later consumed resume submission cannot rebind the originating correlation", async () => {
    ...
    expect(scope?.correlation).toBe(createAssessmentJourneyCorrelation(submissionId, signingSecret));
    expect(scope?.correlation).not.toBe(
      createAssessmentJourneyCorrelation(resumeSubmissionId, signingSecret),
    );
  });

  test("emits only the allowlisted reveal record", () => {
    const correlation = createAssessmentJourneyCorrelation(submissionId, signingSecret);
