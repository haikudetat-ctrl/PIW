...
select has_function('public', 'submit_all_season_lead', array['uuid','uuid','text','text','text','text','text','timestamp with time zone','jsonb','text','text','text','integer','text','text'], 'legacy All Season function remains available with its prior signature');
...
select ok((select position('p_company_id::text || '':all-season-website:'' || p_submission_id::text' in pg_catalog.pg_get_functiondef('public.submit_all_season_lead(uuid,uuid,text,text,text,text,text,timestamptz,jsonb,text,text,text,integer,text,text)'::regprocedure)) > 0), 'legacy All Season replay lock includes the company');
