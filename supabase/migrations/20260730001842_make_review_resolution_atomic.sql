-- Keep the review transition, its required audit row, and any retry event in
-- one database transaction. A successful RPC therefore always leaves durable
-- work for the outbox relay; any context or persistence failure rolls the
-- entire action back.
