...
begin
  ...
  loop
    ...
    insert into public.leadconduit_events (
      ...
      p_company_id, trim(v_event->>'event_id'), trim(v_event->>'flow_id'),
      nullif(trim(v_event->>'source_id'), ''), nullif(trim(v_event->>'source_name'), ''),
      nullif(trim(v_event->>'lead_id'), ''), coalesce(nullif(trim(v_event->>'event_type'), ''), 'unknown'),
      coalesce((v_event->>'occurred_at')::timestamptz, p_observed_at), nullif(trim(v_event->>'outcome'), ''),
      nullif(trim(v_event->>'external_lead_id'), ''), nullif(trim(v_event->>'phone_normalized'), ''),
      nullif(trim(v_event->>'email_normalized'), ''), nullif(trim(v_event->>'raw_status'), ''),
      nullif(trim(v_event->>'step_id'), ''), nullif(trim(v_event->>'step_name'), ''),
      nullif(trim(v_event->>'rule_id'), ''), nullif(trim(v_event->>'rule_name'), ''),
      nullif(trim(v_event->>'rule_scope'), ''), nullif(trim(v_event->>'rule_scope_id'), ''),
      nullif(trim(v_event->>'reason_category'), ''), nullif(trim(v_event->>'lead_name'), ''),
      nullif(trim(v_event->>'submitted_phone'), ''), nullif(trim(v_event->>'submitted_email'), ''),
      nullif(trim(v_event->>'submitted_address'), ''), nullif(trim(v_event->>'campaign'), ''),
      nullif(trim(v_event->>'consent_reference'), ''), nullif(trim(v_event->>'trustedform_url'), ''),
      coalesce(v_event->'attribution', '{}'::jsonb),
      private.redact_credential_values(coalesce(v_event->'raw_payload', '{}'::jsonb)),
      coalesce((v_event->>'is_test')::boolean, false), p_observed_at, array[p_channel]::text[],
      p_observed_at, case when p_channel = 'webhook' then p_observed_at end,
    ...
    set source_id = coalesce(public.leadconduit_events.source_id, excluded.source_id),
        ...
        raw_status = case
          when excluded.event_type = 'shadow_checkpoint' and excluded.raw_status = 'likely_filter_match' then excluded.raw_status
          when public.leadconduit_events.event_type = 'shadow_checkpoint' and public.leadconduit_events.raw_status = 'likely_filter_match' then public.leadconduit_events.raw_status
          else coalesce(public.leadconduit_events.raw_status, excluded.raw_status)
        end,
        ...
        reason_category = case
          when excluded.event_type = 'shadow_checkpoint' and excluded.raw_status = 'likely_filter_match' then excluded.reason_category
          when public.leadconduit_events.event_type = 'shadow_checkpoint' and public.leadconduit_events.raw_status = 'likely_filter_match' then public.leadconduit_events.reason_category
          else coalesce(public.leadconduit_events.reason_category, excluded.reason_category)
        end,
        ...
        attribution = case
          when excluded.event_type = 'shadow_checkpoint' and excluded.raw_status = 'likely_filter_match' then excluded.attribution
          when public.leadconduit_events.event_type = 'shadow_checkpoint' and public.leadconduit_events.raw_status = 'likely_filter_match' then public.leadconduit_events.attribution
          else excluded.attribution || public.leadconduit_events.attribution
        end,
        raw_payload = case
          when excluded.event_type = 'shadow_checkpoint' and excluded.raw_status = 'likely_filter_match' then excluded.raw_payload
          when public.leadconduit_events.event_type = 'shadow_checkpoint' and public.leadconduit_events.raw_status = 'likely_filter_match' then public.leadconduit_events.raw_payload
          else public.leadconduit_events.raw_payload
        end,
        processing_status = case
          when public.leadconduit_events.processing_status in ('processed', 'pending', 'failed') then public.leadconduit_events.processing_status
        ...
        ingestion_channels = array_remove(array[
          case when 'webhook' = any(public.leadconduit_events.ingestion_channels) or p_channel = 'webhook' then 'webhook' end,
          case when 'poll' = any(public.leadconduit_events.ingestion_channels) or p_channel = 'poll' then 'poll' end
        ]::text[], null),
        first_observed_at = least(public.leadconduit_events.first_observed_at, excluded.first_observed_at),
        webhook_received_at = coalesce(least(public.leadconduit_events.webhook_received_at, excluded.webhook_received_at), public.leadconduit_events.webhook_received_at, excluded.webhook_received_at),
        poll_observed_at = coalesce(least(public.leadconduit_events.poll_observed_at, excluded.poll_observed_at), public.leadconduit_events.poll_observed_at, excluded.poll_observed_at);
